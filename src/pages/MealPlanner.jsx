...
const MealPlanner = () => {
  ...
  const fetchRecipes = async () => {
    try {
      ...
      const querySnapshot = await getDocs(query(recipesRef, where("userId", "==", user.uid)));
  ...
  const fetchMealPlan = async () => {
    try {
      ...
      const querySnapshot = await getDocs(query(mealPlanRef, where("userId", "==", user.uid)));
  
      const updatedMealPlan = { ...mealPlan };
  
      querySnapshot.forEach((doc) => {
        const { recipe, days, meal_type } = doc.data();
  
        if (!recipe || !days || !meal_type) return;
        const mealType = meal_type.toLowerCase();
  
        if (!["breakfast", "lunch", "dinner"].includes(mealType)) return;
  
        days.forEach((day) => {
          const formattedDay = day.charAt(0).toUpperCase() + day.slice(1).toLowerCase();
          if (updatedMealPlan[formattedDay]) {
            updatedMealPlan[formattedDay][mealType] = { id: doc.id, ...recipe };
          }
        });
      });
...
const handleDrop = async (e, dropDay, dropMealType) => {
  ...
  const updatedMealPlan = { ...mealPlan };
  updatedMealPlan[dropDay][dropMealType] = selectedRecipe;
  setMealPlan(updatedMealPlan);

  try {
    await addDoc(collection(db, "mealPlans"), {
      userId: user.uid,
      recipe: { name: selectedRecipe.name, category: selectedRecipe.category },
      days: [dropDay],
      meal_type: dropMealType,
      createdAt: new Date(),
    });
  } catch (error) {
